...
| SIGEF — Extratos (detalhamento público) | Parcial e comprovada somente para programa `02` do PDDE Básico. O detalhamento por URL respondeu em três UEx com CNPJ, Banco do Brasil, agência e conta do rótulo exato `PDDE` coincidentes. | Consultar no máximo cinco UEx por execução; normalizar somente banco (3), agência (4), conta BB (10, preservando `X`) e CNPJ (14); salvar HTML/JSON/hash e todas as linhas retornadas. Uma chave auxiliar SHA-256 colapsa apenas linhas idênticas da mesma resposta. | `CREDITO_LOCALIZADO_SIGEF`, `DIVERGENCIA_ENTRE_FONTES` ou `CONSULTA_INCONCLUSIVA`; não inferir programas, contas, saldo ou natureza de despesa. |
