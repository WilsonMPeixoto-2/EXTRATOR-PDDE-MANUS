...
| **Piloto operacional restrito** | SIGEF — Liberações, rota legada | O formulário e o detalhe público retornaram CNPJ, programa, data, OB, valor, banco, agência e C/C sem CAPTCHA. O conector realiza até cinco consultas por execução, preservando HTML/JSON, hash e observações. | Corroborar pagamento apenas por CNPJ, parcela, valor e data; preservar OB, banco, agência e conta como evidência SIGEF independente; manter a interface SIGEF moderna como `CAPTCHA_REQUIRED`. |
...
O teste revelou um caminho público diferente da interface SIGEF moderna que exige CAPTCHA. O formulário legado `internet_fnde.liberacoes_01_pc` declarou em JavaScript o destino `internet_fnde.liberacoes_result_pc`; a listagem municipal retornou entidades e a abertura do detalhe por CNPJ trouxe as colunas de pagamento, OB, valor, programa, banco, agência e conta. Um piloto limitado a cinco UEx corroboradas pelo PDDEInfo obteve cinco respostas HTTP 200, sem marcador CAPTCHA, e cinco coincidências em CNPJ, parcela PDDE Básico, data e valor. A rota acrescentou a OB e os dados bancários em todos os casos. O conector foi então habilitado no fluxo como piloto operacional limitado a cinco UEx com pagamento básico registrado, com pausa entre consultas, retentativas, persistência de HTML/JSON e hash por resposta.

Essa integração continua sendo de **corroboração**, não de preenchimento: um registro SIGEF só poderá abrir observação relacionada a pagamento PDDEInfo quando coincidirem CNPJ, parcela declarada, data e valor. A OB, o banco, a agência e a conta são preservados como atributos da evidência SIGEF e não preenchem ou substituem `Dados Bancários` do PDDEInfo. O estado produzido é **OB corroborada; crédito não localizado**, e não crédito bancário confirmado. O sucesso da rota legada não altera a classificação `CAPTCHA_REQUIRED` das telas SIGEF modernas de Liberações, Conta Corrente e Extratos, nem autoriza sua automação.
