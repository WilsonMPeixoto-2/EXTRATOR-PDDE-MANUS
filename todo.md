...
- [x] Executar o segundo lote de 15 UEx elegíveis no SIGEF, com recorte de 2026, exclusão das UEx já concluídas e preservação integral das evidências. A execução `71a55398-0069-413b-a54e-09a846a29f87` registrou 15 consultas inconclusivas por timeout SSL do SIGEF; nenhuma evidência ou inferência foi criada, e a cobertura acumulada permaneceu em 20 UEx.
- [x] Exibir na auditoria a cobertura acumulada do SIGEF sobre a lista PDDEInfo aprovada, sem substituir os 163 registros da fonte primária. O indicador usa INEPs únicos com evidência SIGEF preservada e informa 20/163 UEx em dois lotes concluídos.
- [x] Realizar análise completa do estado do projeto: arquitetura, fluxo de dados, banco, integrações, segurança, testes, operação, documentação e experiência de auditoria. Relatório consolidado em `ANALISE_GERAL_PROJETO_2026_08_14.md`.
- [x] Aprimorar o indicador visual de cobertura SIGEF na auditoria, evidenciando total coberto, percentual, significado complementar e referência PDDEInfo preservada. O bloco agora apresenta anel percentual, razão de UEx, barra de progresso, lotes, última evidência e ressalva explícita de que não substitui o PDDEInfo.
- [x] Adicionar tooltip acessível ao indicador de cobertura SIGEF com detalhamento de UEx cobertas, pendentes, lotes, última evidência e limites de interpretação. O conteúdo é acessível por mouse e teclado, preserva o contexto de fonte complementar e não sugere ausência de crédito.
- [x] Gerar e entregar extrato exemplificativo completo de uma UEx, limitado às movimentações SIGEF com data em 2026 e fontes rastreáveis. O exemplo do INEP 33069301 contém 5 de 188 movimentações datadas de 2026, com URL, hash e chave do artefato integral preservados.
- [x] Investigar se a ausência de movimentações SIGEF posteriores a maio de 2026 decorre de defasagem da fonte ou de limitação do filtro e da extração. A análise de 20 arquivos integrais comprovou data máxima de 03/05/2026 em conteúdo devolvido pelo SIGEF, sem regra local de corte mensal; a fonte não permite concluir se houve ou não movimentos posteriores.
- [x] Pesquisar documentos, notícias e informes oficiais sobre eventual defasagem do SIGEF e avaliar meios oficiais ou autorizados de obter dados do PDDE com menor atraso. A pesquisa confirmou defasagem no canal público consultado, mas não localizou comunicado FNDE que explique a causa; identificou os dados abertos diários da CGU para transferências/recebimento como piloto prioritário e o BB Gestão Ágil como fonte bancária institucional sujeita a acesso autorizado.
- [x] Corrigir a referência residual a `collectSigefDirectExtractFull` que impede o servidor de carregar o módulo SIGEF sem erro em desenvolvimento. A exportação e a importação atuais estão alinhadas; o log registrava uma versão transitória anterior à recarga do arquivo. A suíte com 102 testes, a tipagem e o build carregaram o módulo sem erro.
- [x] Executar pilotos técnicos auditáveis de fontes públicas e institucionalmente autorizáveis — Portal da Transparência/CGU, Webservice SIGEF, BB Gestão Ágil/API BB e Plataforma Antonieta de Barros — para classificar a viabilidade real de dados financeiros tempestivos do PDDE. O piloto CGU leu 1.014.771 registros de janeiro a julho de 2026 e vinculou 97 transferências PDDE do FNDE a 95 UEx por CNPJ; os demais canais comprovaram infraestrutura, produto ou barreira de credencial, mas não forneceram dados das UEx sem habilitação formal.
- [x] Atualizar o catálogo de fontes e a documentação com resultados reais, campos disponíveis, atualidade observada, chaves de vínculo, barreiras de acesso e limitações de cada piloto. O catálogo classifica a CGU como piloto concluído com limitações, mas mantém a coleta de produção bloqueada até existir adaptador auditável; o dossiê documenta métodos, resultados, URLs e fronteiras de interpretação.
- [x] Estruturar a importação automática e auditável do PDDEInfo e dos arquivos mensais de transferências da CGU, definindo disparo, persistência de fonte, validações, idempotência e apresentação por atualidade. A arquitetura separa fila persistida, evidência bruta, linhas CGU vinculadas por CNPJ, execução PDDEInfo fatiada e rotinas idempotentes; documentada em `ARQUITETURA_IMPORTACAO_AUTOMATICA_2026_08_15.md`.
- [ ] Implementar a primeira versão da importação escolhida: atualização assistida por botão ou atualização automática em segundo plano para PDDEInfo e CGU, sem alterar a hierarquia de fontes ou a semântica financeira.
- [x] Analisar criticamente o relatório externo anexado, validando em código e evidência apenas as recomendações compatíveis com os contratos financeiros e de auditoria do Extrator da 4ª CRE. A análise rejeitou números SIGEF, infraestrutura Supabase/Next/Vercel e Excel de nove abas não reproduzidos ou incompatíveis; preservou como úteis apenas controles específicos já incorporados e a separação rigorosa de fontes.
- [x] Auditar e atualizar ferramentas, dependências e documentação do repositório de forma compatível, com lockfile revisado e validação completa antes da publicação. A auditoria encontrou 91 alertas transitivos, sem atualização semver compatível disponível; foi atualizada a base `baseline-browser-mapping` indicada pelo ambiente. Migrações maiores de Vite, Vitest, Express, React, Recharts, pnpm ou TypeScript foram deliberadamente adiadas por exigirem plano de compatibilidade. Validação: 109 testes, tipagem e build aprovados.
- [x] Avaliar o repositório `pdde-repasse-conciliador` e o comparativo anexado, validando alegações técnicas e classificando padrões reutilizáveis, riscos e incompatibilidades antes de qualquer incorporação ao Extrator da 4ª CRE. A análise confirmou como aproveitáveis centavos inteiros, proteção contra fórmula, cadeia de evidências, guardas SIGEF e publicação apenas após cobertura integral; rejeitou a migração integral, SSE não comprovado e a promoção de resultados SIGEF sem artefatos próprios.
- [x] Incorporar seletivamente os padrões aprovados do repositório comparado: centavos inteiros, neutralização de fórmulas no Excel, guardas de resposta SIGEF e publicação de visão corrente condicionada à execução completa aprovada. O coletor SIGEF agora bloqueia HTML não reconhecido e qualquer detalhamento cuja identidade não coincida com CNPJ, banco, agência, conta e programa consultados; a rota de referência corrente só seleciona execução aprovada com 163/163 registros. Validação: 109 testes, tipagem e build aprovados.
- [x] Incorporar conversão monetária em centavos e neutralização de fórmulas no Excel. O parser agora converte moeda brasileira por centavos inteiros antes de materializar o valor exibido; o workbook prefixa textos externos iniciados por `=`, `+`, `-` ou `@` sem alterar campos numéricos, datas, abas ou fórmulas inexistentes. Cobertura: 107 testes, tipagem e build aprovados.
- [x] Atualizar o diagnóstico factual de cobertura de escolas, contas e parcelas de 2026, distinguindo cadastro no PDDEInfo, pagamento registrado, transferência complementar e crédito bancário efetivamente evidenciado. A execução aprovada de 14/08/2026 cobre 163/163 escolas; confirma contas PDDE Básico em 116, PDDE Qualidade em 163 e PDDE Equidade em 5 UEx; registra R$ 827.615,00 pagos em 169 parcelas, mas a evidência SIGEF complementar cobre apenas 20 UEx e 14 créditos compatíveis.
- [x] Documentar e aplicar a regra de cruzamento: SIGEF e demais fontes só podem complementar uma conta ausente ou confirmar uma conta existente quando CNPJ, programa, banco, agência e conta coincidirem; nunca podem substituir o PDDEInfo por inferência. A conciliação atual já rejeita chaves incompletas e marca divergência se a conta for diferente, ainda que CNPJ e valor coincidam; o parser também proíbe usar PDDE Qualidade ou Equidade para preencher PDDE Básico.
- [x] Identificar e testar fontes institucionais oficiais que aceitem CNPJ ou INEP para localizar a numeração das 47 contas de PDDE Básico ausentes, sem pressupor a conta na consulta. A Pesquisa Conta Corrente do SIGEF aceita CNPJ, mas foi testada com reCAPTCHA na submissão; o BB Gestão Ágil é a rota institucional prioritária, pois o BB declara reunir todas as contas de um beneficiário, porém exige habilitação formal. Portal da Transparência, CGU, SIMAD e extrato SIGEF por rota direta não revelam conta desconhecida.
